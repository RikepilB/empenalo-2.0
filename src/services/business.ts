...
import type { BusinessContext, BusinessProfile } from "@/types";

// Returns the logged-in business owner's business + active subscription.
// Used by the business layout to render the plan / quota widget.
export async function getBusinessContext(): Promise<BusinessContext | null> {
  ...
  const { data: business } = await supabase
    ...
    .eq("owner_id", user.id)
    .maybeSingle<BusinessContext["business"]>();
...
export async function updateBusiness(
  input: z.input<typeof updateBusinessSchema>,
): Promise<{ ok: true }> {
